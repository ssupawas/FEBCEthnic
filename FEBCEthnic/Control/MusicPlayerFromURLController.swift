//
//  MusicPlayerFromURLController.swift
//  FEBCEthnic
//
...
class MusicPlayerFromURLController: UIViewController, WKNavigationDelegate {
    ...
    var song: Song?{
        didSet{
            ...
            guard let songURL = song?.songURL else { return }
            
            
            let songFileURL = URL(string: songURL)!
            let playerItem = AVPlayerItem(url: songFileURL)
            
            player = AVPlayer(playerItem: playerItem)
            player.play()
            
            //           guard let webURL = song?.webURL else { return }
            //            let songWeb = URL(string: webURL)
            ...
            coverImage.image = song?.cover2 //เปลี่ยนดึงรูปจาก cover มาใช้ cover2 แทน เพราะว่าใน cover ถ้าใส่ list รูปที่ไม่เหมือนกันจะทำให้ list แสดงรายชื่อเพลง โชว์แค่ รายการเดียวเท่านั้น
            songName.text = song?.songName
            songArtist.text = song?.artist
            songDays.text = song?.days
    ...
    let coverImage: UIImageView = {
        ...
        image.image = UIImage(named: "coverfebcethnic")
        image.contentMode = .scaleAspectFill
        image.layer.masksToBounds = true
        image.layer.cornerRadius = 220/2
        return image
    }()
...
//        label.font = UIFont.boldSystemFont(ofSize: 22)
        label.font = UIFont.KanitMedium(size: 24)
...
//        label.font = UIFont.boldSystemFont(ofSize: 17)
        label.font = UIFont.KanitLight(size: 19)
        label.numberOfLines = 0
        label.textAlignment = .center
        return label
    }()
    ...
    let datelabel: UILabel = {
        let label = UILabel()
        label.text = "Days"
        label.textColor = UIColor.blackAlpha(alpha: 1.0)
        //        label.font = UIFont.boldSystemFont(ofSize: 16)
        label.font = UIFont.PoppinsLight(size: 16)
        label.numberOfLines = 0
        label.textAlignment = .center
        return label
    }()
    ...
    override func viewDidLoad() {
        ...
        view.backgroundColor = .init(patternImage: UIImage(named: "bg002")!)  // ใส่ BG เป็นภาพ
        //        view.backgroundColor = .white   // ใส่ Bg เป็นสี
...
//        share.tintColor = UIColor.mediumBlue
//
...
//        navigationItem.rightBarButtonItem = share
        ...
        view.addSubview(webView)   //Web
        view.addSubview(datelabel)
        
        
        let coverImagePadding: CGFloat = ( view.frame.width - 220 ) / 2
        
        coverImageShadow.anchor(view.safeAreaLayoutGuide.topAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, topConstant: 10, leftConstant: coverImagePadding, bottomConstant: 0, rightConstant: coverImagePadding, widthConstant: 220, heightConstant: 220)
        coverImage.anchor(view.safeAreaLayoutGuide.topAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, topConstant: 10, leftConstant: coverImagePadding, bottomConstant: 0, rightConstant: coverImagePadding, widthConstant: 220, heightConstant: 220)
        songName.anchor(coverImage.bottomAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, topConstant: 20, leftConstant: 50, bottomConstant: 0, rightConstant: 50, widthConstant: 0, heightConstant: 0)
        ...
        webView.anchor(songDays.bottomAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, topConstant: 10, leftConstant: 50, bottomConstant: 0, rightConstant: 50, widthConstant: 0, heightConstant: 0)
        datelabel.anchor(webView.bottomAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, topConstant: 20, leftConstant: 50, bottomConstant: 0, rightConstant: 50, widthConstant: 0, heightConstant: 0)
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        loadRequest(urlStr: song!.webURL)
